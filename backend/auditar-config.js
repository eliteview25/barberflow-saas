...
for(const [k,def] of [['PLAN_STARTER_PRICE',39.90],['PLAN_PRO_PRICE',69.90],['PLAN_PREMIUM_PRICE',119.90]]){const v=Number(process.env[k]??def);if(!Number.isFinite(v)||v<=0||v>100000)erro(`${k} inválido`)}
