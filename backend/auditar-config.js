...
if(prod&&!process.env.LEGAL_ENTITY_NAME)aviso('LEGAL_ENTITY_NAME ausente: publique a identificação real do fornecedor antes da venda pública');
if(prod&&!process.env.LEGAL_CNPJ)aviso('LEGAL_CNPJ ausente: informe CNPJ/CPF quando aplicável à oferta eletrônica');
if(prod&&!process.env.LEGAL_ADDRESS)aviso('LEGAL_ADDRESS ausente: informe endereço do fornecedor para transparência contratual');
if(prod&&!process.env.PRIVACY_EMAIL&&!process.env.SUPPORT_EMAIL)aviso('PRIVACY_EMAIL/SUPPORT_EMAIL ausentes: falta canal público de privacidade/LGPD');
...
for(const k of ['JWT_SECRET','APP_SECRETS_ENCRYPTION_KEY','BOOKING_OTP_PEPPER','LOGIN_THROTTLE_SECRET','CRON_SECRET','MP_WEBHOOK_TENANT_SECRET','BILLING_WEBHOOK_SECRET','BACKUP_ENCRYPTION_KEY'])if(prod&&process.env[k]&&/change.?me|troque|example|barberflow|eliteflow/i.test(process.env[k]))erro(`${k} parece usar valor previsível/de exemplo`);
