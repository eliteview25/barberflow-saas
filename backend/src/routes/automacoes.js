...
async function candidates(kind){
  const s=spec(kind);return (await pool.query(`SELECT a.id,a.barbearia_id,a.data,a.horario,c.nome cliente,c.telefone,b.nome barbeiro,sv.nome servico,
    ac.whatsapp_provedor,ac.${s.template} template_nome,ac.${s.message} mensagem
    FROM agendamentos a
...
async function processKind(kind){
  let ok=0,fail=0,skip=0;const cache=new Map();
  for(const raw of await candidates(kind)){
    const x={...raw,kind};let claim=null;
    try{
      claim=await claimDelivery(x,kind);if(!claim){skip++;continue}
      let integ=cache.get(x.barbearia_id);if(integ===undefined){integ=await wp.activeConnection(x.barbearia_id);cache.set(x.barbearia_id,integ||null)}if(!integ)throw new Error('Nenhum provedor WhatsApp ativo');
      const data=String(x.data).slice(0,10).split('-').reverse().join('/'),hora=String(x.horario).slice(0,5),fallback=interpolate(x.mensagem,x);
      if(integ.provedor==='evolution')await wp.sendText(integ,x.telefone,fallback);
      else{
        if(!x.template_nome){await pool.query(`UPDATE automacoes_envios SET status='erro',erro='Template/Content SID não configurado',proxima_tentativa=NOW()+INTERVAL '1 day',atualizado_em=NOW() WHERE id=$1`,[claim.id]);skip++;continue}
        await sendTemplate(integ,x.telefone,x.template_nome,[x.cliente,x.servico,x.barbeiro,data,hora],'pt_BR',{fallbackText:fallback});
      }
    ...
    }catch(e){fail++;if(claim)await pool.query(`UPDATE automacoes_envios SET status='erro',erro=$1,proxima_tentativa=CASE WHEN tentativas<5 THEN NOW()+(LEAST(60,POWER(2,tentativas)::int)*INTERVAL '1 minute') ELSE NULL END,atualizado_em=NOW() WHERE id=$2`,[String(e.message).slice(0,500),claim.id]);console.error('Automação',kind,x.id,e.message)}
  }
