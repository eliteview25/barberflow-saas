...
router.get('/callback',async(req,res)=>{try{const{code,state,error,error_description}=req.query;if(error)return res.redirect(`/pages/configuracoes.html?mp=erro&motivo=${encodeURIComponent(error_description||error)}`);if(!code||!state)throw new Error('Código OAuth ausente');await concluirConexao({code,state});res.redirect('/pages/configuracoes.html?mp=conectado')}catch(e){console.error('OAuth Mercado Pago:',e.data||e);res.redirect(`/pages/configuracoes.html?mp=erro&motivo=${encodeURIComponent(e.message||'Falha ao conectar')}`)}});
