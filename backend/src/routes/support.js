...
router.post('/tickets',async(req,res)=>{try{const categoria=['acesso','pagamento','agendamento','configuracao','privacidade','outro'].includes(String(req.body?.categoria))?String(req.body.categoria):'outro';const assunto=cleanText(req.body?.assunto,160,{required:true}),mensagem=cleanText(req.body?.mensagem,3000,{required:true});if(!assunto||!mensagem)return res.status(400).json({erro:'Informe assunto e mensagem'});const r=await pool.query(`INSERT INTO support_tickets(barbearia_id,usuario_id,categoria,assunto,mensagem) VALUES($1,$2,$3,$4,$5) RETURNING id,categoria,assunto,status,prioridade,criado_em`,[req.usuario.barbearia_id,req.usuario.id,categoria,assunto,mensagem]);const tenant=(await pool.query(`SELECT nome FROM barbearias WHERE id=$1`,[req.usuario.barbearia_id])).rows[0];notificar('support_ticket_created',{barbearia_id:req.usuario.barbearia_id,barbearia:tenant?.nome,ticket_id:r.rows[0].id,assunto}).catch(()=>{});res.status(201).json({mensagem:'Chamado enviado ao suporte',ticket:r.rows[0]})}catch(e){console.error(e);res.status(500).json({erro:'Erro ao abrir chamado'})}});
