...
router.post('/mercadopago',async(req,res)=>{try{
  ...
  const eventId=mpEventId(req,type,dataId);let barbeariaId=null;if(type==='payment'||type==='payments'){const candidate=Number(req.query.barbearia_id);if(!Number.isSafeInteger(candidate)||candidate<1||!validarMpTenantSignature(candidate,req.query.tenant_sig))return res.status(401).json({erro:'Roteamento do webhook inválido'});barbeariaId=candidate;}const row=await enqueue('mercadopago',eventId,{type,dataId,barbeariaId});if(row.status!=='processado')kick('mercadopago',eventId);return res.sendStatus(200);
