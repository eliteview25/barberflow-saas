...
function verifyTotp(secret,code,window=1){const c=String(code||'').replace(/\D/g,'');if(c.length!==6)return false;const now=Date.now();for(let i=-window;i<=window;i++){const x=totp(secret,now+i*30000);const a=Buffer.from(x),b=Buffer.from(c);if(a.length===b.length&&crypto.timingSafeEqual(a,b))return true}return false}
