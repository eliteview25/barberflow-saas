async function notificar(evento, dados) {
  ...
  try {
    await fetch(url, { method:'POST', headers:{'Content-Type':'application/json','x-barberflow-secret':process.env.AUTOMATION_WEBHOOK_SECRET||''}, body:JSON.stringify({evento,dados,ocorreu_em:new Date().toISOString()}) });
