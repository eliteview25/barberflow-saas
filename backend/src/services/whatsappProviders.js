...
async function save(barbeariaId,provider,{numero=null,config={},secretValue=null,status='conectado',webhookToken=null}={}){
  ...
  const r=await pool.query(`INSERT INTO whatsapp_conexoes(barbearia_id,provedor,numero,config,secret_enc,webhook_token_hash,webhook_token_enc,status,conectado_em,atualizado_em)
    VALUES($1,$2,$3,$4,$5,$6,$7,$8,CASE WHEN $8='conectado' THEN NOW() ELSE NULL END,NOW())
...
async function jsonFetch(url,opts={}){const r=await fetch(url,{...opts,signal:externalSignal()});let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d?.error?.message||d?.message||d?.detail||`Provedor respondeu ${r.status}`);return d}
...
function normalizeResult(provider,d){const id=d?.messages?.[0]?.id||d?.sid||d?.key?.id||d?.response?.key?.id||null;return{provider,messages:id?[{id:String(id)}]:[],raw:d}}
