...
const PROVIDERS={
  mercadopago:{id:'mercadopago',nome:'Mercado Pago',checkout:true,metodos:['Pix','Cartão'],descricao:'Salve as credenciais da conta Mercado Pago da barbearia. O segredo do webhook permite confirmação automática mesmo quando o cliente fecha a página.',campos:[{id:'access_token',label:'Access Token',tipo:'password',required:true,placeholder:'APP_USR-...'},{id:'public_key',label:'Public Key',tipo:'text',required:true,placeholder:'APP_USR-...'},{id:'webhook_secret',label:'Segredo do webhook',tipo:'password',required:false,placeholder:'Opcional, mas recomendado para confirmação automática'}]},
...
function credentialsFor(provedor,raw={}){
  ...
  if(provedor==='mercadopago'){
    const access_token=clean(raw.access_token,1000),public_key=clean(raw.public_key,500),webhook_secret=clean(raw.webhook_secret,1000);
    ...
    if(webhook_secret&&webhook_secret.length<16)invalid('O segredo do webhook parece inválido');
    return {secret:{access_token,...(webhook_secret?{webhook_secret}:{})},publicKey:public_key,environment:/TEST|test/.test(access_token)?'sandbox':'production',accessToken:access_token};
  }
  if(provedor==='pagbank'){
...
async function saveProviderCredentials(barbeariaId,provedor,raw){
  ...
  if(provedor==='mercadopago'&&!c.secret.webhook_secret){
    const old=(await pool.query(`SELECT secret_enc FROM integracoes_pagamento WHERE barbearia_id=$1 AND provedor='mercadopago' LIMIT 1`,[barbeariaId])).rows[0];
    if(old?.secret_enc){try{const prev=JSON.parse(decrypt(old.secret_enc)||'{}');if(prev.webhook_secret)c.secret.webhook_secret=prev.webhook_secret}catch{}}
  }
  ...
  await pool.query(`INSERT INTO integracoes_pagamento(barbearia_id,provedor,access_token_enc,refresh_token_enc,secret_enc,public_key,environment,metadata,capabilities,status,last_verified_at,last_error,conectado_em,atualizado_em)
    ...
    ON CONFLICT(barbearia_id,provedor) DO UPDATE SET access_token_enc=EXCLUDED.access_token_enc,refresh_token_enc=NULL,secret_enc=EXCLUDED.secret_enc,public_key=EXCLUDED.public_key,environment=EXCLUDED.environment,metadata=EXCLUDED.metadata,capabilities=EXCLUDED.capabilities,status=EXCLUDED.status,last_verified_at=NULL,last_error=NULL,conectado_em=NOW(),atualizado_em=NOW()`,[
      barbeariaId,provedor,accessTokenEnc,secretEnc,c.publicKey||null,c.environment,JSON.stringify({credential_fields:fields,source:'owner_credentials'}),JSON.stringify({credentials_ready:true,checkout_driver:provedor==='mercadopago',webhook_ready:provedor==='mercadopago'&&!!c.secret.webhook_secret}),status
    ]);
...
async function listGateways(barbeariaId){
  ...
  return Object.values(PROVIDERS).map(p=>{const x=by.get(p.id)||{},saved=['conectado','credenciais_salvas'].includes(x.status),caps=x.capabilities||{};return {...p,credenciais_salvas:saved,conectado:p.id==='mercadopago'&&x.status==='conectado',status:x.status||'sem_credenciais',ambiente:x.environment||'production',atualizado_em:x.atualizado_em||null,checkout_ativo:p.checkout&&x.status==='conectado',webhook_ready:!!caps.webhook_ready};});
}


async function tenantWebhookSecret(barbeariaId){
  if(!Number.isSafeInteger(Number(barbeariaId))||Number(barbeariaId)<1)return null;
  const r=await pool.query(`SELECT secret_enc FROM integracoes_pagamento WHERE barbearia_id=$1 AND provedor='mercadopago' AND status='conectado' LIMIT 1`,[Number(barbeariaId)]);
  if(!r.rowCount||!r.rows[0].secret_enc)return null;
  try{const data=JSON.parse(decrypt(r.rows[0].secret_enc)||'{}');const secret=String(data.webhook_secret||'').trim();return secret.length>=16?secret:null}catch{return null}
}
...
module.exports={PROVIDERS,credentialsFor,ensurePaymentGatewaySchema,listGateways,saveProviderCredentials,disconnectGateway,tenantWebhookSecret};
