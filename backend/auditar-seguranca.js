...
check(/getPlatformMercadoPagoCredentials/.test(tenant) && /status:\s*'pending'/.test(mp) && /mp\.init_point/.test(tenant) && /window\.location\.assign\(r\.checkout_url\)/.test(front) && !/sdk\.mercadopago\.com/.test(app), 'Checkout de cartão usa init_point oficial do Mercado Pago sem coletar cartão no BarberFlow');
