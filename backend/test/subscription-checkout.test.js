...
test('CSP libera Secure Fields do Mercado Pago para o formulário de cartão',()=>{const app=read('src/app.js');assert.match(app,/https:\/\/secure-fields\.mercadopago\.com/);assert.match(app,/frameSrc:\[[^\]]*secure-fields\.mercadopago\.com/)});
test('checkout de cartão evita renderização concorrente e mostra estado de carregamento',()=>{const f=fs.readFileSync(path.join(front,'js/assinatura.js'),'utf8');const h=fs.readFileSync(path.join(front,'pages/assinatura.html'),'utf8');assert.match(h,/id="cardLoading"/);assert.match(f,/cardRenderPromise/);assert.match(f,/selectMethod\('card',false\)/);assert.match(h,/Carregando formulário seguro do Mercado Pago/)});
