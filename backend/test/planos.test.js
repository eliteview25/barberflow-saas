...
test('Starter contém somente núcleo esperado',()=>{const r=recursosDoPlano('starter');for(const x of ['agenda','clientes','barbeiros','servicos'])assert.ok(r.includes(x));assert.ok(r.includes('pagina_publica_simples'));assert.ok(!r.includes('equipe'));});
test('Pro inclui equipe e financeiro básico',()=>{const r=recursosDoPlano('pro');assert.ok(r.includes('equipe'));assert.ok(r.includes('financeiro_basico'));assert.ok(r.includes('pdv_estoque'));assert.ok(!r.includes('automacoes'));});
