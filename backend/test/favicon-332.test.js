...
test('favicon EliteFlow existe nos formatos principais',()=>{
...
test('todas as páginas HTML usam favicon versionado',()=>{
  ...
  for(const p of pages){
    ...
    if(!/rel="icon"[^>]+favicon\.svg\?v=20260901-v453/i.test(html)) missing.push(path.relative(frontend,p));
    if(!/rel="apple-touch-icon"[^>]+apple-touch-icon\.png\?v=20260901-v453/i.test(html)) missing.push(`${path.relative(frontend,p)} (apple)`);
  }
  assert.deepEqual(missing,[]);
});

test('favicon SVG preserva cores da identidade EliteFlow',()=>{
