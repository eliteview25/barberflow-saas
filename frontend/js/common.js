...
function brandLockup(extra=''){return `<span class="bf-brand-lockup ${extra}"><img src="/favicon.svg?v=20260901-v453" alt=""><span class="bf-brand-word">Elite<span>Flow</span></span></span>`}
...
function userAvatarHtml(u=currentUser(),extra=''){const photo=safeClientUrl(u?.foto_url);const initials=String(u?.nome||'EF').trim().split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'EF';return `<span class="bf-user-avatar ${extra}">${photo?`<img src="${esc(photo)}" alt="Foto de ${esc(u?.nome||'usuário')}">`:`<span>${esc(initials)}</span>`}</span>`}
...
async function api(path,opts={},retried=false){
  let r;try{r=await fetch(`${API}${path}`,{...opts,credentials:'same-origin',headers:authHeaders(opts.headers||{})})}catch{throw new Error('Não foi possível conectar ao EliteFlow. Verifique sua internet e tente novamente.')}
  let d={};try{d=await r.json()}catch{}
...
function decoratePremiumTopbar(){
  ...
  const search=document.createElement('div');search.className='bf-topbar-search';search.innerHTML=`${iconSVG('search',17)}<input type="search" id="bfGlobalSearch" autocomplete="off" placeholder="Buscar clientes, agenda, recursos..." aria-label="Buscar no EliteFlow"><span class="bf-search-shortcut">⌘ K</span><div class="bf-search-results hidden" id="bfGlobalSearchResults"></div>`;
