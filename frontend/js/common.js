...
function safeClientUrl(v){try{const u=new URL(String(v||''),location.origin);return ['http:','https:'].includes(u.protocol)&&!u.username&&!u.password?u.href:''}catch{return ''}}
...
function decoratePremiumTopbar(){
  ...
  const account=document.createElement('a');account.className='bf-topbar-account';account.href='/pages/configuracoes.html?secao=perfil';account.innerHTML=`${userAvatarHtml(u,'top')}<span><strong>${esc(u.nome||'Usuário')}</strong><small>${esc(roleLabel(u.papel))}</small></span><b>⌄</b>`;
