...
function renderShell(active){
  ...
  const menu=[
    ...
    linkHtml(byKey.agendamentos),
    linkHtml(byKey.clientes),
    groupHtml('operacao','receipt','Operação',['gestao-comandas','gestao-pdv','gestao-vendas','servicos','gestao-produtos','pagina-publica']),
    groupHtml('equipe','users','Equipe',['barbeiros','gestao-comissoes','equipe']),
    linkHtml(byKey.financeiro),
    groupHtml('crescimento','trend','Crescimento',['gestao-crm','marketing-oportunidades','marketing-resumo','marketing-campanhas','marketing-publicos','marketing-cupons','marketing-indicacoes','marketing-links','marketing-modelos','gestao-fidelidade','gestao-clube','gestao-fila']),
    linkHtml(byKey.automacoes),
    groupHtml('relatorios','chart','Relatórios',['gestao-bi','gestao-relatorios','gestao-avaliacoes','gestao-dados','gestao-fiscal']),
    groupHtml('configuracoes','settings','Configurações',['pagamentos','seguranca','perfil']),
  ...
  return `
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-search"><span>${iconSVG('search',16)}</span><input id="menuSearch" type="search" placeholder="Buscar no BarberFlow" autocomplete="off"></div>
      <a class="sidebar-create" href="/pages/agendamentos.html#novo">${iconSVG('plus',16)}<span>Novo agendamento</span></a>
