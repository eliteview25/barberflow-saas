if(requireAuth(['dono','gerente'])){
  ...
  async function load(){try{dados=await api('/usuarios');usuarios.innerHTML=dados.map(x=>`<tr><td>${esc(x.nome)}</td><td>${esc(x.email)}</td><td><span class="badge">${esc(roleLabel(x.papel))}</span></td><td>${esc(x.barbeiro_nome||'-')}</td><td><span class="badge">${x.ativo?'Ativo':'Inativo'}</span></td><td class="actions">${owner?`<button class="btn btn-secondary" data-click="editUser(${x.id})">Editar</button><button class="btn ${x.ativo?'btn-danger':'btn-success'}" data-click="toggleUser(${x.id},${!x.ativo})">${x.ativo?'Desativar':'Ativar'}</button>`:'Somente leitura'}</td></tr>`).join('')||'<tr><td colspan="6">Nenhum usuário.</td></tr>';}catch(e){flash(msg,e.message,'error')}}
