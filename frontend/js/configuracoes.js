if(requireAuth(['dono','gerente'])){
  ...
  el.pagamento_agendamento.onchange=toggleSinal;el.aceitar_pix_manual.onchange=togglePixManual;ids.forEach(id=>{if(el[id]&&!['pagamento_agendamento','aceitar_pix_manual'].includes(id))el[id].addEventListener(el[id].type==='checkbox'?'change':'input',preview)});el.tema.addEventListener('change',preview);
  async function loadMP(){try{const st=await api('/mercadopago/status');mpStatus.textContent=st.conectado?'Conectado':'Não conectado';mpStatus.className=`badge ${st.conectado?'status-concluido':'status-cancelado'}`;conectarMP.classList.toggle('hidden',st.conectado);desconectarMP.classList.toggle('hidden',!st.conectado);if(user.papel!=='dono'){conectarMP.disabled=true;desconectarMP.disabled=true}}catch(e){mpStatus.textContent='Indisponível';console.error(e)}}
  conectarMP.onclick=async()=>{try{const r=await api('/mercadopago/conectar');location.href=r.url}catch(e){flash(msg,e.message,'error')}};
  desconectarMP.onclick=async()=>{if(!confirm('Desconectar a conta Mercado Pago desta barbearia?'))return;try{await api('/mercadopago/conexao',{method:'DELETE'});el.aceitar_mercadopago.checked=false;await loadMP();flash(msg,'Mercado Pago desconectado')}catch(e){flash(msg,e.message,'error')}};
  async function carregarPixPendentes(){try{const list=await api('/pagamentos-pendentes');pixPendentes.innerHTML=list.length?list.map(x=>`<div class="pending-payment-row"><div><strong>${esc(x.nome)}</strong><div class="muted">${esc(x.servico)} • ${dateBR(x.data)} ${timeBR(x.horario)} • ${money(x.valor_cobrado)}</div><small>${esc(x.telefone)}</small></div><button class="btn btn-primary" data-confirmar-pix="${x.id}">Confirmar recebimento</button></div>`).join(''):'<p class="muted">Nenhum Pix manual aguardando confirmação.</p>';pixPendentes.querySelectorAll('[data-confirmar-pix]').forEach(b=>b.onclick=async()=>{if(!confirm('Você conferiu o recebimento deste Pix?'))return;try{await api(`/pagamentos-pendentes/${b.dataset.confirmarPix}/confirmar`,{method:'POST'});flash(msg,'Pix confirmado e agendamento criado');carregarPixPendentes()}catch(e){flash(msg,e.message,'error')}})}catch(e){pixPendentes.innerHTML=`<p class="muted">${esc(e.message)}</p>`}}
  atualizarPixPendentes.onclick=carregarPixPendentes;
