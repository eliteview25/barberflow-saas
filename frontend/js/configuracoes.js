if(requireAuth(['dono','gerente'])){
  ...
  function setAiDisabled(disabled){[aiEls.nome,aiEls.tom,aiEls.inicio,aiEls.fallback,aiEls.consultar,aiEls.agendar,aiEls.reagendar,aiEls.cancelar,aiEls.precos,aiEls.pagamento,aiEls.humano,aiEls.reclamacao,aiEls.problemaPagamento,aiEls.salvar].forEach(x=>{if(x)x.disabled=disabled})}
  function fillAi(c={}){aiEls.nome.value=c.nome_assistente||'Sofia';aiEls.tom.value=c.tom||'amigavel';aiEls.inicio.value=c.mensagem_inicial||'';aiEls.fallback.value=c.mensagem_fallback||'';aiEls.consultar.checked=c.consultar_horarios!==false;aiEls.agendar.checked=c.criar_agendamento!==false;aiEls.reagendar.checked=c.reagendar!==false;aiEls.cancelar.checked=c.cancelar!==false;aiEls.precos.checked=c.informar_precos!==false;aiEls.pagamento.checked=c.enviar_link_pagamento!==false;aiEls.humano.checked=c.transferir_solicitacao!==false;aiEls.reclamacao.checked=c.transferir_reclamacao!==false;aiEls.problemaPagamento.checked=c.transferir_pagamento!==false}
  async function loadAi(){try{const d=await api('/ia/status');fillAi(d.config||{});aiEls.status.textContent=d.motor_ativo?'IA ativa':'Preparada';aiEls.status.className=`badge ${d.recurso_disponivel?'status-concluido':'status-cancelado'}`;if(d.recurso_disponivel){aiEls.hint.className='notice success';aiEls.hint.textContent='Premium: configuração liberada. O motor de IA ainda não envia mensagens; será conectado na próxima etapa.';setAiDisabled(false)}else if(d.addon_elegivel){aiEls.hint.className='notice';aiEls.hint.textContent='Plano Pro: a IA está prevista como adicional. Faça upgrade para Premium para já preparar o assistente.';setAiDisabled(true)}else{aiEls.hint.className='notice';aiEls.hint.textContent='A preparação da IA está disponível no Premium.';setAiDisabled(true)}}catch(e){aiEls.status.textContent='Indisponível';aiEls.hint.className='notice error';aiEls.hint.textContent=e.message;setAiDisabled(true)}}
  aiEls.salvar.onclick=async()=>{try{aiEls.salvar.disabled=true;aiEls.salvar.textContent='Salvando...';const body={nome_assistente:aiEls.nome.value,tom:aiEls.tom.value,mensagem_inicial:aiEls.inicio.value,mensagem_fallback:aiEls.fallback.value,consultar_horarios:aiEls.consultar.checked,criar_agendamento:aiEls.agendar.checked,reagendar:aiEls.reagendar.checked,cancelar:aiEls.cancelar.checked,informar_precos:aiEls.precos.checked,enviar_link_pagamento:aiEls.pagamento.checked,transferir_solicitacao:aiEls.humano.checked,transferir_reclamacao:aiEls.reclamacao.checked,transferir_pagamento:aiEls.problemaPagamento.checked};await api('/ia/config',{method:'PUT',body:JSON.stringify(body)});flash(msg,'Preparação da IA salva. Próximo passo: conectar o motor ao WhatsApp.')}catch(e){flash(msg,e.message,'error')}finally{aiEls.salvar.disabled=false;aiEls.salvar.textContent='Salvar preparação da IA'}};
